...
addEventListener("DOMContentLoaded", function () {
    ...
    let valorDelDolar = localStorage.getItem("valorDelDolar") ? parseFloat(localStorage.getItem("valorDelDolar")) : 0;
    ...
    const formBuscadorOtroCliente = document.getElementById("form-buscador-otro-cliente");
    ...
    const inputTotalDeConfirmacion = document.getElementById("inputTotalDeConfirmacion");
    ...
    const btnInsertarServicioModal = document.getElementById("btnInsertarServicioModal");
    ...
    const buscarPaciente = async (formularioPaciente) => {
        try {
            ...
            let resultado = await executePetition("/Sistema-del--CEM--JEHOVA-RAFA/Factura/mostrarPaciente", "POST", datos);
    ...
    const createPatients = async (form, inputs) => {
        try {
            ...
            if (result.ok) {
                ...
                inputCedulaPaciente.dispatchEvent(new Event("keyup", { bubbles: true }));
    ...
    const buscarPacienteConCita = async (formularioPaciente) => {
        try {
            ...
            let resultado = await executePetition("/Sistema-del--CEM--JEHOVA-RAFA/Factura/mostrarPacienteConCita", "POST", datos);
    ...
    const buscarPacienteConHospit = async (id) => {
        try {
            let resultado = await executePetition("/Sistema-del--CEM--JEHOVA-RAFA/Factura/datosHospitalizacion/" + id, "GET");

            if (!resultado.length > 0) {
                alertError("Error", "Lamentablememte no hay hospitalizaciones con ese numero");
                return;
            }
    ...
    const addServicioTable = (id) => {
        ...
        if (filterService) {
            ...
            alertError("Error", "No puede agregar el mismo servicio con el mismo doctor.");
            return;
        }
    ...
    const traerServiciosMedicos = async () => {
        try {
            const result = await executePetition(`/Sistema-del--CEM--JEHOVA-RAFA/Factura/mostrarServicios`, "GET");
            console.log(result);
    ...
    const traerInsumos = async () => {
        try {
            const result = await executePetition(`/Sistema-del--CEM--JEHOVA-RAFA/Factura/mostrarInsumos`, "GET");
    ...
    const insertarInsumoSeleccionado = (id, cantidad, nombre, precio, iva, medida) => {
        const obj = {
            id_insumo: id,
            cantidad: cantidad,
            nombre: nombre,
        ...
        const insumoSeleccionado = dataInsumo.find((insumo) => insumo.id_insumo == id);

        //lista para restar
        const insumosARestar = listaModalInsumo.find((insumo) => insumo.id_insumo == id);

        const insumosRegistrados = dataInsumo.find((insumo) => insumo.id_insumo == id);

        insumosARestar ? (insumosARestar.cantidad_disponible -= cantidad) : 0;

        insumoSeleccionado ? (insumosRegistrados.cantidad += cantidad) : dataInsumo.push(obj);
    ...
    function calcularTotal() {
        ...
        let total = parseFloat(totalFactura) + parseFloat(subTotal) + parseFloat(insumos);
    ...
    function mostrarInsumo() {
        ...
        dataInsumo.forEach((element, index) => {
            let storedDolar = localStorage.getItem("valorDelDolar");
            let montoBSSubTotal =
                (parseFloat(element["precio"]) * parseInt(element["cantidad"]) + parseFloat(element["iva"])) * storedDolar;
    ...
    const mostrarTiposDePago = async () => {
        try {
            const result = await executePetition("/Sistema-del--CEM--JEHOVA-RAFA/Factura/mostrarMetodosDePago", "GET");
            let html = "";
            console.log(result);
            if (result.length > 0) {
    ...
    const checkearTiposDePago = (metodosPago) => {
        ...
        if (efectivo.checked && pagoMovil.checked == false && transferencia.checked == false && divisa.checked == false) {
            ...
            console.log(inputsDeValidacion);
    ...
    referencia.addEventListener("keyup", function () {
        ...
        if (
            inputsDeValidacion[0].parentElement.classList.contains("d-none") &&
            inputsDeValidacion[1].parentElement.classList.contains("d-none") &&
            inputsDeValidacion[2].parentElement.classList.contains("d-none")
        ) {
            return;
        }
