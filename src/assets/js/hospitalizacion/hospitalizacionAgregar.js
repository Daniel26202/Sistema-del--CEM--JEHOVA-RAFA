...
let inputCedula = document.querySelector("#bt");
...
const traerInsumos = async () => {
    try {
        ...
        let resultadoInsu = await executePetition(url + "/serviciosDH/" + valorI, "GET");
...
const traerControlDePaciente = async () => {
    ...
    let resultadoVP = await executePetition(url + "/validarPaciente", "POST", data);

    // llamo la función mostrar
    let resultadoM = await executePetition(url + "/mostrarInformacionPCD", "POST", data);
