...
inicializarValidacionFormulario(document.getElementById("buscadoresEspecialidades"));

///filtro

document.getElementById("sintomas").addEventListener("click", function () {
    const inputs = this.closest(".modal-content").querySelectorAll(".input-validar");
    if (!inputs[0].parentElement.classList.contains("valido") && !inputs[1].parentElement.classList.contains("valido")) {
        Swal.fire({
            icon: "error",
            title: "Error",
    ...
    sintomas_chart(`/Sistema-del--CEM--JEHOVA-RAFA/Inicio/sintomas_comunes_filtrados/${inputs[0].value}/${inputs[1].value}`);
...
const especialidades_chart = async (url) => {
    try {
        ...
        if (data.length > 0) {
            ...
            let colorTextoInicial = localStorage.getItem("theme") === "dark" ? "#e0e1dd" : "#1b1a1a";
...
document.getElementById("reporte").addEventListener("hidden.bs.modal", () => {
    const inputs = document.getElementById("buscadoresEspecialidades").querySelectorAll(".input-validar");
...
document.getElementById("reporteSintomas").addEventListener("hidden.bs.modal", () => {
    const inputs = document.getElementById("buscadoresSintomas").querySelectorAll(".input-validar");
    inputs.forEach((input) => {
        let campoCustom = input.closest(".campo-custom");
        let spamP = campoCustom.querySelector(".icono-der");
        let check = spamP.children[0];
...
window.addEventListener("themeChanged", () => {
    ...
    const nuevoColor = localStorage.getItem("theme") === "dark" ? "#e0e1dd" : "#1b1a1a";
