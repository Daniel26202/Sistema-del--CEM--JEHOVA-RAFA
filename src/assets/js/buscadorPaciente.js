addEventListener("DOMContentLoaded", function () {
  ...
  const traerDatos = async (cedula) => {
    try {
      ...
      let peticion = await fetch(
        "?c=ControladorPacientes/mostrarPaciente",
        contenido
      );
      ...
      if (resultado.length > 0) {
        resultado.forEach((res) => {
          ...
          document.getElementById("id_pacienteBuscador").value =
            res.id_paciente;
          document.getElementById("id_pacienteEliminarBuscador").value =
            res.id_paciente;
          ...
          document.getElementById("apellidoEditarBuscador").value =
            res.apellido;
          document.getElementById("telefonoEditarBuscador").value =
            res.telefono;
          document.getElementById("direccionEditarBuscador").value =
            res.direccion;
          document.getElementById("fnEditarBuscador").value = res.fn;
          document.getElementById("id_patologiaEditarBuscador").value =
            res.id_patologia;
          document.getElementById("patologiaEditarBuscador").value =
  ...
  const traerTodasLasPatologias = async (cedula, positivo) => {
    let peticion = await fetch(
      "?c=ControladorPacientes/mostrarPatologias&cedula=" + cedula
    );
    let resultado = await peticion.json();
