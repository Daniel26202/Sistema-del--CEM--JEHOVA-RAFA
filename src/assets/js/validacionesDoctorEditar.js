    ...
    document.querySelectorAll('.editar').forEach(ele => {
        ele.addEventListener('click', function () {
            ...
            document.querySelector(`#${idModal} .uk-modal-dialog .formulario_editar`).addEventListener('submit', (e) => {
                ...
                if (camposEditar.cedula && camposEditar.nombre && camposEditar.horas && camposEditar.dia && camposEditar.apellido && camposEditar.telefono && camposEditar.email) {


                    document.querySelector(`#${idModal} .uk-modal-dialog .formulario_editar`).submit()
                    mens.forEach(msj => { msj.classList.add("d-none"); });

                } else {
                    e.preventDefault();
                    mens.forEach(msj => { msj.classList.remove("d-none"); });
