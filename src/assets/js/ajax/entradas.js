  ...
  const readEntrada = async () => {
    try {
        ...
        { data: "numero_de_lote" },
        {
          ...
          render: function (data, type, row) {
            return `  <div class="d-flex justify-content-center align-items-center">
                                    ...
                                    <div class="me-2">
                                        <button class="${
                                          !urlActual.includes("papelera")
                                            ? ""
                                            : "d-none"
                                        } btn btnEditarDoctor btn-tabla btn-dt-tabla mb-1 btn-js editar botonesEdi" data-bs-toggle="modal" data-bs-target="#exampleModalagregarEntrada" data-index='${row.id_entrada}' data-insumo=${row.id_insumo} data-proveedor=${row.id_proveedor} data-precio=${row.precio_entrada}>
                    ...
                    }" data-index="${row.id_entrada}" title="Restablecer Entrada" uk-tooltip=""  aria-describedby="uk-tooltip-27">
     ...
     const asignarEventos = ()=>{
       ...
       document.querySelectorAll(".botonesEdi").forEach((btn) => {
         btn.addEventListener("click", function () {
           ...
           const parametros = {
             ...
             modal:
               formAgregarEntrada.parentElement.parentElement.parentElement,
             btnModal: botonModal,
      ...
      // re-inicializa
      initDataTable(selector,`${url}/${metodo}`,columnsEntradas,(datosServer)=>{console.log(datosServer);
      }, asignarEventos);
    } catch (error) {
