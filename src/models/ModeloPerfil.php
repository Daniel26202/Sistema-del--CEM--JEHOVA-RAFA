...
class ModeloPerfil extends ModelBase
{
	...
	public function seleccionarUsuarioApk()
	{
		...
		$alias = ['u', 'p', 'e'];
		$unions = ['p.usuario = u.id_usuario', 'e.id_especialidad = p.id_especialidad'];
		$this->set_tables(["segurity.usuario", 'bd.personal', 'bd.especialidad']);
		$this->set_colums(['u.id_usuario', 'u.usuario', 'u.correo', 'u.imagen', 'u.id_rol', 'p.nombre', 'p.apellido', 'p.cedula', 'p.telefono', 'p.nacionalidad', 'p.tipodecategoria', 'e.nombre AS especialidad']);
	...
	public function seleccionarUsuario()
	{
		...
		$alias = ['u', 'p'];
		$unions = ['p.usuario = u.id_usuario'];
		$this->set_tables(["segurity.usuario", 'bd.personal']);
		$this->set_colums(['u.usuario as user']);
		$this->set_alias($alias);
		$this->set_union($unions);
		$this->set_condicion_aditional($coditions);
